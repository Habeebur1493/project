...
const Home = () => {
  ...
  const category = search.icludes ("=") ? decodeURIComponent(search.split("=")[1]);

  const [filteredProducts, setfilteredProducts] = useState(products);

  const getproductsBycategory = async () => {
    if(!category || category === "undefined") return setfilteredProducts(products);
    try {
  ...
  useEffect(() => {
    if (category.length > 0){ getproductsBycategory();
    } else {
